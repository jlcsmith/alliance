...
public class MgmpTransformerTest {

  private static final List<Serializable> CATEGORY_LIST =
      Arrays.asList("imageryBaseMapsEarthCover", "intelligenceMilitary");

  private static final List<String> KEYWORD_LIST =
      Arrays.asList(
          "Mapping and navigation", "Hydrography", "Standard Series", "Raster", "Georeferenced");
  ...
  private static final String ISR_COMMENTS_2 = "optical reflective surface mgmp image description";
  ...
  private static final String OFFICIAL_SENSITIVE = "OFFICIAL-SENSITIVE";
  ...
  private MgmpTransformer mgmpTransformer;
  ...
  public void setUp() {
    ...
    MgmpConverter mgmpConverter = mock(MgmpConverter.class);
    when(mgmpConverter.getSecurityMap())
        .thenReturn(MgmpUtility.securityMappingListToMap(MgmpUtilityTest.SECURITY_LIST));
    mgmpTransformer = new MgmpTransformer(MGMP_METACARD_TYPE, mockAttributeRegistry, mgmpConverter);
  }

  @Test
  public void testInputTransformerWithFullMgmpMetacard() throws Exception {
    InputStream fileInput = MgmpTransformerTest.class.getResourceAsStream("/full_data_mgmp.xml");
    ...
    assertThat(
        convertDate((Date) metacard.getAttribute(MgmpConstants.PUBLISHED_DATE).getValue()),
        is("1998-01-01 00:00:00 UTC"));

    Date createdDate = (Date) metacard.getAttribute(Core.METACARD_CREATED).getValue();
    Date modifiedDate = (Date) metacard.getAttribute(Core.METACARD_MODIFIED).getValue();
    ...
    assertThat(metacard.getAttribute(Core.LANGUAGE).getValues(), hasItems("ger", "eng"));

    assertThat(metacard.getContentTypeName(), is(SERVICE));

    assertThat(metacard.getAttribute(Core.DATATYPE).getValue(), is(SERVICE));

    List<Serializable> countryCodes = metacard.getAttribute(Location.COUNTRY_CODE).getValues();
    ...
    assertThat(metacard.getAttribute(Core.DESCRIPTION).getValue(), is(METACARD_DESCRIPTION));

    assertThat(
        metacard.getAttribute(Isr.CATEGORY).getValues(),
        hasItems(ISR_CATEGORY_1, ISR_CATEGORY_2, ISR_CATEGORY_3));

    assertThat(
        metacard.getAttribute(Isr.COMMENTS).getValues(),
        hasItems(ISR_COMMENTS_1, ISR_COMMENTS_2, ISR_COMMENTS_3, ISR_COMMENTS_4));

    List<Serializable> keywords = metacard.getAttribute(Topic.KEYWORD).getValues();
    assertThat(
        keywords,
        hasItems(
            KEYWORD_LIST.get(0),
    ...
    assertThat(
        metacard.getAttribute(Contact.PUBLISHER_NAME).getValues(), hasItem(ORGANISATION_NAME));

    assertThat(
        metacard.getAttribute(Contact.PUBLISHER_PHONE).getValues(), hasItem("+44 (0)321 321 1122"));
    ...
    assertThat(metacard.getAttribute(Media.FORMAT).getValue(), is("s57"));

    assertThat(metacard.getAttribute(Media.FORMAT_VERSION).getValue(), is("3.1"));
    ...
    assertThat(
        metacard.getAttribute(GmdConstants.RESOURCE_STATUS).getValues(),
        hasItems("planned", "completed"));

    assertThat(
        metacard.getAttribute(Isr.NATIONAL_IMAGERY_INTERPRETABILITY_RATING_SCALE).getValue(),
        is(2.0));

    assertThat(metacard.getAttribute(Security.METADATA_CLASSIFICATION).getValue(), is(OFFICIAL));

    assertThat(
        metacard.getAttribute(Security.METADATA_RELEASABILITY).getValues(),
        hasItems(SECURITY_LIST.get(0), SECURITY_LIST.get(1)));

    assertThat(
        metacard.getAttribute(Security.METADATA_DISSEMINATION).getValues(),
        hasItems(MgmpConstants.RELEASABLE_TO));

    assertThat(
        metacard.getAttribute(Security.METADATA_ORIGINATOR_CLASSIFICATION).getValue(),
        is(UNCLASSIFIED));

    assertThat(
        metacard.getAttribute(Security.RESOURCE_CLASSIFICATION).getValue(), is(OFFICIAL_SENSITIVE));

    assertThat(
        metacard.getAttribute(Security.RESOURCE_RELEASABILITY).getValues(),
        hasItems(SECURITY_LIST.get(0), SECURITY_LIST.get(1)));
    assertThat(
        metacard.getAttribute(Security.RESOURCE_DISSEMINATION).getValues(),
        hasItems(MgmpConstants.RELEASABLE_TO));

    assertThat(
        metacard.getAttribute(Security.RESOURCE_ORIGINATOR_CLASSIFICATION).getValue(),
        is(NATO_SECRET));

    assertThat(metacard.getAttribute(Isr.CLOUD_COVER).getValue(), is(13.0));

    assertThat(
        metacard.getAttribute(Associations.RELATED).getValues(),
        hasItem("00000000-0000-0000-0000-000000000077"));

    List<Serializable> categories = metacard.getAttribute(Topic.CATEGORY).getValues();
    assertThat(categories, hasItems(CATEGORY_LIST.get(0), CATEGORY_LIST.get(1)));

    assertThat(
        metacard.getAttribute(Isr.DATA_QUALITY).getValues(),
        hasItems(
            "Resource Grade : Something statement",
  ...
  public void testInputTransformerWithEmptyMgmpMetacard() throws Exception {
    InputStream fileInput = MgmpTransformerTest.class.getResourceAsStream("/empty_mgmp.xml");
    Metacard metacard = mgmpTransformer.transform(fileInput);
  ...
  public void testInputTransformerWithSampleMgmpMetacard() throws Exception {
    InputStream fileInput = MgmpTransformerTest.class.getResourceAsStream("/sample_mgmp_1.xml");
    Metacard metacard = mgmpTransformer.transform(fileInput, METACARD_ID);

    assertThat(
        metacard.getAttribute(Security.RESOURCE_CLASSIFICATION).getValue(), is(OFFICIAL_SENSITIVE));
    assertThat(metacard.getAttribute(Security.RESOURCE_RELEASABILITY).getValues(), hasItems("UK"));
    assertThat(
        metacard.getAttribute(Security.RESOURCE_DISSEMINATION).getValues(),
        hasItems(MgmpConstants.EYES_DISCRETION));
    assertThat(
        metacard.getAttribute(Security.RESOURCE_ORIGINATOR_CLASSIFICATION).getValue(),
        is(NATO_SECRET));
    ...
    assertThat(
        metacard.getAttribute(Security.METADATA_DISSEMINATION).getValues(),
        hasItems(MgmpConstants.EYES_ONLY));
    assertThat(
        metacard.getAttribute(Security.METADATA_ORIGINATOR_CLASSIFICATION).getValue(),
        is(UNCLASSIFIED));

    assertThat(
        metacard.getAttribute(Core.LANGUAGE).getValues(), is(MgmpConstants.DEFAULT_LANGUAGE));
    assertThat(
        metacard.getAttribute(Isr.NATIONAL_IMAGERY_INTERPRETABILITY_RATING_SCALE), nullValue());
  ...
  public void testInputTransformerWithSampleMgmpMetacardEdgeCases() throws Exception {
    InputStream fileInput = MgmpTransformerTest.class.getResourceAsStream("/sample_mgmp_2.xml");
    Metacard metacard = mgmpTransformer.transform(fileInput, METACARD_ID);

    assertThat(metacard.getAttribute(Security.RESOURCE_CLASSIFICATION).getValue(), is("unmapped"));
  ...
  public static MetacardType getMgmpMetacardType() {
